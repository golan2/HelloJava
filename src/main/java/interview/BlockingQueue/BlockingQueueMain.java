...
public class BlockingQueueMain {
    ...
    private static CountDownLatch waiter = new CountDownLatch(5);		//wait for all 5 consumers/producers to end

    public static void main(String[] args) throws InterruptedException, BrokenBarrierException {

        final FixedSizeBlockingQ<Integer> queue = new FixedSizeBlockingQ<>(5);
        final ExecutorService executor = Executors.newFixedThreadPool(5, new ProConFactory());
    ...
    private static class BQLog {
        ...
        private static void println(String message) {
            ...
            System.out.printf("%s [%d] %s\n", simpleDateFormat.format(calendar.getTime()), Thread.currentThread().getId(), message);
        }
    }
